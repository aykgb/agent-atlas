...
需要 uv（Python 3.10 或更新版本，缺失时由 uv 自动安装）；SQLite 需支持 FTS5 全文搜索扩展及 trigram 分词器。dsh 会话为 zstd 压缩，需要系统有 `zstd` 命令（macOS 可用 `brew install zstd`）；缺失时跳过 dsh 并在页脚提示。
...
首次启动全量建立索引；之后点击「刷新数据」按增量更新：只重解析有变化的 session 文件（meta 记录毫秒水位 `indexed_at`，结合 `files` 表的大小判断），并同步启用中的远端。完成前仍可查询旧索引。`.venv/bin/python stats_server.py --reindex` 强制全量重建。
...
索引含完整搜索文本和字符三元组全文索引，体积可能为数百 MB。少于三个字符的搜索使用字面扫描，保证中文短词可匹配。输入词频倒排表为 `terms(term, turn_id, count)`。刷新为增量：只重解析 mtime 晚于水位或大小变化的文件，删除已消失文件的行；用量按文件记录后在查询前聚合。首次运行、schema 升级与 `--reindex` 走全量重建（临时文件后原子替换）。远端数据不写入本机索引，各自独立存放；同步按 `cursor` + 轮次指纹增量，远端重建导致 id 重排时自动全量重拉，失败整事务回滚保留上次数据，状态记录在该远端库的 `meta`。
...
测试覆盖六类日志的归属与去重、思考计量、输入输出配对、中文搜索与提示词长度过滤、词频范围、日历边界、本机增量更新（水位与 size 判断、文件删除）、dsh 的 zstd 解压与用量口径（缺失 zstd 时降级告警）、远端指纹增量与重建回退、同步失败保留、查询合并与轮次路由，以及 HTTP 的 Host / Origin 白名单与远端只读边界。
