...
测试覆盖六类日志的归属与去重、思考计量、输入输出配对、中文搜索与提示词长度过滤、词频范围、日历边界、本机增量更新（水位与 size 判断、文件删除、自动检查的挂钟计时）、dsh 的 zstd 解压与用量口径（缺失 zstd 时降级告警）、远端指纹增量与重建回退、同步失败保留、查询合并与轮次路由、会话列表分组排序分页与跨来源路由，以及 HTTP 的 Host / Origin 白名单、远端只读边界与静态资源的 ETag / 304。
